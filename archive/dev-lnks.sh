...
_instapaper() {
    ...
    credentialstring="instapaper_username=\"$instapaper_username\""
		credentialstring="$credentialstring\ninstapaper_password=\"$instapaper_password\""
		echo -en "$credentialstring" >> ~/.lnks.conf
	}
...
_instapaper_curl() {
	...
	if [[ $lnx == "Error: No matching links" ]]; then
		...
		links | while read -r url; do
			instapaperstring="username=$instapaper_username&password=$instapaper_password&url=$url"
			curl -d "$instapaperstring" https://www.instapaper.com/api/add > /dev/null 2>&1
			echo "$url Saved!"
			sleep .2
		done
	fi
...
# Not Implemented:
# _pinboard() {
#	 curl -k --get "https://api.pinboard.in/v1/posts/add/?auth_token=USER:TOKEN" \ 
#    --data-urlencode "url=URL" -d "description=DESC" -d "tags=TAGS"
# }
...
# use getopts instead of case statement
